...
const TaskManager = require("./libs/TaskManager.js");
const odmInfo = require("./libs/odmInfo.js");
const si = require("systeminformation");
const S3 = require("./libs/S3.js");

const auth = require("./libs/auth/factory.js").fromConfig(config);
const authCheck = auth.getMiddleware();
const taskNew = require("./libs/taskNew.js");
...
app.post("/task/chin", authCheck, (req, res) => {
  const inputFile = path.join("data", "odm_mesh.ply"); // Ruta del archivo de entrada (ply)
  const outputFile = path.join("data", "odm_mesh.fbx"); // Ruta del archivo de salida (fbx)

  // Verifica si el archivo de entrada existe
  if (!fs.existsSync(inputFile)) {
    return res.status(404).json({ error: "El archivo .ply no existe" });
  }

  // Comando de Assimp para convertir el archivo
  ...
  exec(command, (error, stdout, stderr) => {
    ...
    // Si todo va bien, responde con éxito y el archivo convertido
    console.log(`Archivo convertido exitosamente: ${stdout}`);
    res.json({
      success: true,
      message: "Archivo convertido a .fbx",
      file: outputFile,
    });
  });
});
...
app.get("/task/:uuid/model", authCheck, getTaskFromUuid, (req, res) => {
  const convertFile = req.task.getConvertScriptPath();
  const pat2Model = req.task.getModelPathTwo();

  const folder2 = req.task.getModelFolderPathTwo();

  const inputFile = pat2Model;
  const outputFile = path.join(folder2, "odm_textured_model_geo.fbx");
  const texturing = path.join(folder2, "odm_textured_model_geo_material0000_map_Kd.png");
  ...
  exec(blenderCommand, (err, stdout, stderr) => {
    if (err) {
      ...
      return res.status(500).json({ error: "Error al convertir el archivo" +err});
    }

    if (err) {
      console.error(`Error al convertir el archivo: ${stderr}`);
      return res.status(500).json({ error: "Error al convertir el archivo" +stderr});
    }
